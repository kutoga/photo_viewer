...
const { CACHE_VERSION, VIDEOS, hasGPS, parseLocation } = require('./core.cjs');
  ...
  const resize = (pipeline) =>
    preview === 'full'
      ...
      : preview
        ...
        : pipeline.resize(320, 320, { fit: 'inside', withoutEnlargement: true });
  try {
    return await resize(sharp(file, { failOn: 'none' }).rotate())
...
async function processMedia({ file, id, stat, cacheDir }) {
  ...
  try {
    ...
    const dimensions = await renderImage(type === 'video' ? frame : file, temp);
    await fs.rename(temp, dest);
    entry.hasThumbnail = true;
    entry.thumbnailWidth = dimensions.width;
    entry.thumbnailHeight = dimensions.height;
    delete entry.error;
  } catch (err) {
...
async function makePreview(entry, cacheDir, full = false) {
  const dest = path.join(cacheDir, 'previews', `${entry.id}_${full ? 'full' : 'preview'}.jpg`);
  if (await validImage(dest)) return dest;
  const temp = dest + '.tmp.jpg';
  try {
    if (entry.type === 'video') await videoFrame(entry.originalPath, temp, 1600);
    else await renderImage(entry.originalPath, temp, full ? 'full' : true);
    await fs.rename(temp, dest);
    return dest;
  } finally {

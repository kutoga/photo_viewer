...
class Library extends EventEmitter {
  ...
  async storageStats() {
    const bytes = { database: 0, thumbnails: 0, previews: 0 };
    const measure = async (dir, category = 'database') => {
      for await (const entry of await fs.opendir(dir)) {
        const file = path.join(dir, entry.name);
        const group =
          dir === this.cacheDir && entry.isDirectory() && entry.name in bytes
            ? entry.name
            : category;
        try {
          if (entry.isDirectory()) await measure(file, group);
          else if (entry.isFile()) bytes[group] += (await fs.stat(file)).size;
        } catch (err) {
          if (err.code !== 'ENOENT') throw err;
        }
      }
    };
    await measure(this.cacheDir);
    let originalBytes = 0;
    for (const entry of this.entries.values())
      if (Number.isFinite(entry.fileSize)) originalBytes += entry.fileSize;
    return {
      ...this.summary(),
      ...bytes,
      totalBytes: bytes.database + bytes.thumbnails + bytes.previews,
      originalBytes,
    };
  }
  snapshot() {
    ...
    return {
      ...
      needsRescan:
        ...
        [...this.entries.values()].some(
          (entry) => entry.cacheVersion !== CACHE_VERSION || !entry.contentHash,
        ),
    };
  }
  persist() {
    ...
    if (!this.processing.has(id)) {
      const task = this.workers
        ...
        .then(async (entry) => {
          ...
          return { ...entry, contentHash: hash.digest('hex') };
        })

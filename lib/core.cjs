...
async function atomicWrite(filename, data) {
  ...
  try {
    if (Array.isArray(data)) {
      // Keep large scan checkpoints from blocking IPC with one huge stringify.
      const handle = await fs.open(temp, 'w');
      try {
        await handle.writeFile('[', 'utf8');
        for (let i = 0; i < data.length; i += 256) {
          const encoded = [];
          for (let j = i; j < Math.min(i + 256, data.length); j++)
            encoded.push(JSON.stringify(data[j]) ?? 'null');
          await handle.writeFile((i ? ',' : '') + encoded.join(','), 'utf8');
        }
        await handle.writeFile(']', 'utf8');
      } finally {
        await handle.close();
      }
    } else await fs.writeFile(temp, JSON.stringify(data), 'utf8');
